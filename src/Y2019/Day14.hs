...
oreNeeded "ORE" n = bool Nothing (Just n) <$> extract "ORE" n
oreNeeded chem'  n = do
    have <- amount chem'
    if have >= n
        then do extract chem' n >> pure (Just 0)
        else do ore <- acquire chem' $ n - have
                oreNeeded chem' n >> pure ore
...
amount chem' = fromMaybe 0 . M.lookup chem' <$> (cargo <$> get)
...
recipe = do input' <- some quantity
            (chem', i) <- string "=>" >> quantity
            pure $ Recipe input' chem' i


quantity :: Parser (Chemical, Int)
quantity = do n <- whitespace >> natural
              c <- whitespace >> some (satisfy isAsciiUpper)
              many (oneOf ", ") >> pure (c, n)
